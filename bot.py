...
# ================= 페이드 =================
async def fade_out(vc, duration=1.5):
...
async def fade_in(vc, duration=1.5):
    if not vc or not vc.source:
        return
    for i in range(0, 11):
        vc.source.volume = i / 10
...
class SeekSelect(discord.ui.Select):
    def __init__(self):
        super().__init__(
            ...
            options=[discord.SelectOption(label=f"{i*10}%", value=str(i/10)) for i in range(0,11)]
        )
...
class ControlView(discord.ui.View):
    def __init__(self, state="play"):
        ...
        self.add_item(discord.ui.Button(emoji=emoji, style=discord.ButtonStyle.success, custom_id="play_btn", row=0))
        self.children[-1].callback = self.pause

        self.add_item(discord.ui.Button(emoji="⏮️", style=discord.ButtonStyle.secondary, custom_id="back_btn", row=0))
        self.children[-1].callback = self.back

        self.add_item(discord.ui.Button(emoji="⏭️", style=discord.ButtonStyle.secondary, custom_id="skip_btn", row=0))
...
class SearchModal(discord.ui.Modal, title="🎵 검색"):
    ...
    async def on_submit(self, i):
        ...
        for idx, r in enumerate(results):
            btn = discord.ui.Button(label=str(idx+1), style=discord.ButtonStyle.green)

            async def callback(interaction, r=r):
